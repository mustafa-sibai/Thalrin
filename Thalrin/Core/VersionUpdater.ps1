...
$VERSION_FILE = "Thalrin/Version.h"
...
if ($content -match '#define THALRIN_BUILD_DATE "([^"]+)"') {
    $CURRENT_DATE = $matches[1]
} else {
    Write-Error "Could not find THALRIN_BUILD_DATE"
    exit 1
}

# Extract current build number
if ($content -match '#define THALRIN_BUILD_NUMBER (\d+)') {
    $CURRENT_BUILD = [int]$matches[1]
} else {
    Write-Error "Could not find THALRIN_BUILD_NUMBER"
    exit 1
}
...
if ($CURRENT_DATE -eq $TODAY) {
    ...
    $content = $content -replace "(#define THALRIN_BUILD_NUMBER) \d+", "`$1 $NEW_BUILD"
    Write-Host "Build number incremented: $CURRENT_BUILD -> $NEW_BUILD"
} else {
    # Different date, update date and reset build number
    $content = $content -replace '(#define THALRIN_BUILD_DATE) "[^"]+"', "`$1 `"$TODAY`""
    $content = $content -replace "(#define THALRIN_BUILD_NUMBER) \d+", "`$1 0"
